...
fn test_http() {
	...
	t.execute_with(|| {
		// when
		let price = OcwDemo::fetch_n_parse();
		println!("{:?}",price);
		// then
...
fn price_oracle_response(state: &mut testing::OffchainState) {
	state.expect_request(testing::PendingRequest {
		method: "GET".into(),
		uri: HTTP_REMOTE_REQUEST.into(),
		headers: vec![("User-Agent".into(), "jimmychu0807".into())],
		response: Some(br#"{"data":{"id":"polkadot","rank":"4","symbol":"DOT","name":"Polkadot","supply":"957220447.3054800000000000",
"maxSupply":null,"marketCapUsd":"16372478310.0252989298756301","volumeUsd24Hr":"668156844.5803929698188770",
"priceUsd":"17.1041878139073137","changePercent24Hr":"-6.9588137438135807","vwap24Hr":null,
"explorer":"https://polkascan.io/polkadot"},"timestamp":1611642493324}"#.to_vec()),
		sent: true,
		..Default::default()
	});
}