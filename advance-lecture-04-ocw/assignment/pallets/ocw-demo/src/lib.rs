...
// {"data":{"id":"polkadot","rank":"4","symbol":"DOT","name":"Polkadot","supply":"957220447.3054800000000000",
...
// "explorer":"https://polkascan.io/polkadot"},"timestamp":1611642493324}
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			if modu == 3 {
				...
				let result = Self::fetch_dot_price_and_submit_tx();
				if let Err(e) = result {
...
impl<T: Trait> Module<T> {
	...
	fn fetch_dot_price_and_submit_tx() -> Result<(), Error<T>> {
		...
		if let Ok(_guard) = lock.try_lock() {
			match Self::fetch_n_parse() {
				Ok(number) => {
					return Self::offchain_unsigned_tx_signed_payload(number);
				}
	...
	fn fetch_n_parse() -> Result<u32, Error<T>> {
		...
		let price = match Self::parse_price(resp_str) {
			Some(price) => Ok(price),
			None => {
				debug::warn!("Unable to extract price from the response: {:?}", resp_str);
...
/// Returns `None` when parsing failed or `Some(price in cents)` when parsing is successful.
	fn parse_price(price_str: &str) -> Option<u32> {
		...
		if !price.contains(&'.') {
			let price = vec_char_to_vec_u8(price);
			return 	Some(price as u32)
		} else {
			let mut iter = price.split( |&x| x == '.');
			let front = iter.next().unwrap().to_vec();
			let end = iter.next().unwrap().to_vec();

			let price:u32 = vec_char_to_vec_u8(front) as u32;
			let fraction = vec_char_to_vec_u8(end.clone());

			let exp = end.len().checked_sub(2).unwrap_or(0);
			let new = price as u32 * 100 + (fraction / 10_u64.pow(exp as u32)) as u32;
			debug::info!("{:?} {:?} {:?} {:?}",price, fraction, new,exp);
	...
	fn offchain_signed_tx(number: u32) -> Result<(), Error<T>> {
	...
	//不耗gas费，又可以校验签名
	fn offchain_unsigned_tx_signed_payload(number: u32) -> Result<(), Error<T>> {
...
fn vec_char_to_vec_u8(chars : Vec<char>) -> u64 {
	let new :Vec<u8> = chars.into_iter().map(|val| val as u8 -48).collect();
	let mut value= 0u64;
	let len = new.len();
	for (i,v) in new.into_iter().enumerate() {
		if i == len -1 {
			...
			value += (v as u64) * 10u64.pow((len -1 -i) as u32);
		}
	}
	value
}
...
pub fn vec_to_int(digits: impl IntoIterator<Item = char>) -> Option<u64> {
	const RADIX: u32 = 10;

	digits
		.into_iter()
		.map(|c| c.to_digit(RADIX))
		.try_fold(0u64, |ans, i| i.map(|i| ( ans * RADIX as u64) + i as u64))
}
